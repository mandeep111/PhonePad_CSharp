...
{
    private readonly IReadOnlyDictionary<char, PhoneKey> _map;

    public OldPhonePadKeys()
    {
        _map = new Dictionary<char, PhoneKey>
        {
    ...
    public PhoneKey GetKey(char digit) => _map.TryGetValue(digit, out var k)
        ? k : throw new ArgumentException($"Invalid key: {digit}");
}